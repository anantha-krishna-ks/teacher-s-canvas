...
import type { LessonPlanFormData, GeneratedPlan } from "@/utils/generateMockPlan";

interface ReadOnlyLessonPlanProps {
...
export default function ReadOnlyLessonPlan({ data, plan, onBack }: ReadOnlyLessonPlanProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Header card */}
      <motion.article custom={0} variants={sectionVariants} initial="hidden" animate="visible" className="bg-card border border-border rounded-xl p-6">
      ...
      {/* Learning Objectives */}
      <motion.section custom={1} variants={sectionVariants} initial="hidden" animate="visible" className="bg-card border border-border rounded-xl p-6" aria-label="Learning Objectives">
      ...
      {/* Resources */}
      <motion.section custom={2} variants={sectionVariants} initial="hidden" animate="visible" className="bg-card border border-border rounded-xl p-6" aria-label="Required Resources">
        <SectionHeader icon={Package} number={3} title="Required Resources" subtitle="PDFs, Videos, Images" />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 ml-11">
          {plan.resources.map((res, i) => {
            const typeKey = res.type as keyof typeof RESOURCE_ICON_MAP;
            const style = RESOURCE_ICON_MAP[typeKey] || RESOURCE_ICON_MAP.pdf;
            const ResIcon = style.icon;
            return (
              <a key={i} href={res.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-lg border border-border bg-background hover:bg-muted/50 transition-colors group cursor-pointer">
      ...
      {/* Procedure */}
      <motion.section custom={3} variants={sectionVariants} initial="hidden" animate="visible" className="bg-card border border-border rounded-xl p-6" aria-label="Procedure">
      ...
      {/* Assessment */}
      <motion.section custom={4} variants={sectionVariants} initial="hidden" animate="visible" className="bg-card border border-border rounded-xl p-6" aria-label="Assessment">
                ...
                <div key={qi} className="bg-muted/40 border border-border rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2" role="list">
                    {q.options.map((opt, oi) => (
                      <div key={oi} role="listitem" className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm border transition-colors ${
                        oi === q.correctAnswer
      ...
      {/* Homework */}
      <motion.section custom={5} variants={sectionVariants} initial="hidden" animate="visible" className="bg-card border border-border rounded-xl p-6" aria-label="Homework">
        <SectionHeader icon={Home} number={6} title="Follow-up / Homework" />
        <ol className="space-y-2.5 ml-11 list-none">
          {plan.homework.map((item, i) => (
            <li key={i} className="flex items-start gap-2.5 text-sm text-foreground/85">
              <span className="w-5 h-5 rounded bg-muted text-muted-foreground text-xs font-medium flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
              {item}
            </li>
          ))}
        </ol>
